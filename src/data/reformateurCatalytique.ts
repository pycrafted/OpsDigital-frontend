/** Données partagées pour Réformateur catalytique (tableau + graphique). */
...
const allSubRows = REFORMATEUR_CATEGORIES.flatMap((c) => c.subRows);

/** Liste des indicateurs pour le graphique : { key (category_subRow), label } */
export function getReformateurIndicateurOptions(): { key: string; label: string }[] {
...
export function createInitialReformateurData(): HourRow[] {
  ...
  REFORMATEUR_HOURS.forEach((hour) => {
