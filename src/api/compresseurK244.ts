/**
 * API client pour le tableau Compresseur K 244.
 */
 ...
 */
export async function fetchCompresseurK244ByDate(
  date: string
): Promise<HourRow[]> {
  const res = await fetch(
    `${API_BASE}/api/compresseur-k244/by-date/?date=${encodeURIComponent(date)}`,
 ...
 */
export async function fetchCompresseurK244ByDateRange(
  start: string,
  end: string
): Promise<Record<string, HourRow[]>> {
  const res = await fetch(
    `${API_BASE}/api/compresseur-k244/by-date-range/?start=${encodeURIComponent(start)}&end=${encodeURIComponent(end)}`,
    { headers: getAuthHeaders() }
  );
  if (!res.ok) {
    throw new Error(`API error: ${res.status}`);
...
export async function saveCompresseurK244Bulk(
  ...
  const res = await fetch(`${API_BASE}/api/compresseur-k244/bulk-by-date/`, {
    method: 'POST',
    headers: {
