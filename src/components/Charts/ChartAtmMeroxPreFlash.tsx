...
function getEmptyWeekData(): { categories: string[]; series: { name: string; data: number[] }[] } {
  ...
  const hourSuffixes = ATM_MEROX_HOURS.map((h) => atmMeroxHourLabels[h]);
...
const ATM_MEROX_HOURS_PER_DAY = ATM_MEROX_HOURS.map((h) => atmMeroxHourLabels[h]);
...
function getEmptyMonthData(selectedMonth?: string): { categories: string[]; series: { name: string; data: number[] }[] } {
  ...
  for (let dayIndex = 0; dayIndex < daysInMonth; dayIndex++) {
    ...
    ATM_MEROX_HOURS_PER_DAY.forEach((hourLabel) => {
...
const ChartAtmMeroxPreFlash: React.FC<ChartAtmMeroxPreFlashProps> = ({
  ...
  const chartData = useMemo(() => {
      ...
      if (weekAtmMeroxData && weekAtmMeroxData.dates.length > 0) {
        ...
        weekAtmMeroxData.dates.forEach((date, dayIndex) => {
          ...
          ATM_MEROX_HOURS.forEach((h) => {
            categories.push(`${dayLabel} ${atmMeroxHourLabels[h]}`);
        ...
        monthAtmMeroxData.dates.forEach((date) => {
          ...
          const dateLabel =
            parts.length === 3 && parts.every((n) => Number.isFinite(n))
              ...
              : date;
          ATM_MEROX_HOURS.forEach((h) => {
            categories.push(`${dateLabel} ${atmMeroxHourLabels[h]}`);
            const row = dayRows.find((r) => r.hour === h);
            const v = row ? parseValue(row.values[selectedIndicateur] ?? '') : 0;
            chartValues.push(v);
          });
    ...
    const categories = ATM_MEROX_HOURS.map((h) => atmMeroxHourLabels[h]);
  ...
  }, [data, duration, selectedIndicateur, indicateurLabel, selectedMonthProp, selectedQuarterProp, selectedSemesterProp, selectedYearProp, monthAtmMeroxData, weekAtmMeroxData, quarterAtmMeroxData, semesterAtmMeroxData, yearAtmMeroxData]);
