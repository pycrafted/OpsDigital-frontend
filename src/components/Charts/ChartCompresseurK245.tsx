...
const indicateurOptions = getCompresseurK245IndicateurOptions();
const HOUR_COLORS = { h7: '#EA580C', h11: '#0EA5E9', h15: '#059669', h19: '#8B5CF6', h23: '#DC2626', h3: '#F59E0B' };

/** Données hebdomadaires Compresseur K245 : 7 dates + lignes par date (6 créneaux par jour). */
export interface WeekCompresseurK245Data {
  dates: string[];
  rowsByDate: Record<string, HourRow[]>;
}

/** Données mensuelles Compresseur K245 : une date par jour du mois + lignes par date (6 créneaux par jour). */
export interface MonthCompresseurK245Data {
  dates: string[];
  rowsByDate: Record<string, HourRow[]>;
}

/** Données trimestrielles Compresseur K245 : toutes les dates du trimestre + lignes par date (6 créneaux par jour). */
export interface QuarterCompresseurK245Data {
  dates: string[];
  rowsByDate: Record<string, HourRow[]>;
}

/** Données semestrielles Compresseur K245 : toutes les dates du semestre (6 mois) + lignes par date (6 créneaux par jour). */
export interface SemesterCompresseurK245Data {
  dates: string[];
  rowsByDate: Record<string, HourRow[]>;
}

/** Données annuelles Compresseur K245 : toutes les dates de l'année (12 mois) + lignes par date (6 créneaux par jour). */
export interface YearCompresseurK245Data {
  dates: string[];
  rowsByDate: Record<string, HourRow[]>;
}

export interface ChartCompresseurK245Props {
  data: HourRow[];
...
function getEmptyWeekData(): { categories: string[]; series: { name: string; data: number[] }[] } {
  ...
  const hourSuffixes = COMPRESSEUR_K245_HOURS.map((h) => compresseurK245HourLabels[h]);
...
const COMPRESSEUR_K245_HOURS_PER_DAY = COMPRESSEUR_K245_HOURS.map((h) => compresseurK245HourLabels[h]);
...
function getEmptyMonthData(selectedMonth?: string): { categories: string[]; series: { name: string; data: number[] }[] } {
  ...
  for (let dayIndex = 0; dayIndex < daysInMonth; dayIndex++) {
    ...
    COMPRESSEUR_K245_HOURS_PER_DAY.forEach((hourLabel) => {
...
const ChartCompresseurK245: React.FC<ChartCompresseurK245Props> = ({
  data,
  duration,
  ...
  const indicateurLabel = getFieldLabel('compresseur-k245', selectedIndicateur, indicateurOptions.find((o) => o.key === selectedIndicateur)?.label ?? selectedIndicateur);
  ...
  const chartData = useMemo(() => {
    ...
    if (duration === 'semester') {
      if (semesterCompresseurK245Data && semesterCompresseurK245Data.dates.length > 0) {
        const categories: string[] = [];
        const chartValues: number[] = [];
        semesterCompresseurK245Data.dates.forEach((date) => {
          const dayRows = semesterCompresseurK245Data.rowsByDate[date] ?? [];
          const parts = date.split('-').map(Number);
          const year = parts[0];
          const month = parts[1];
          const day = parts[2];
    ...
    if (duration === 'year') {
      if (yearCompresseurK245Data && yearCompresseurK245Data.dates.length > 0) {
        const categories: string[] = [];
        const chartValues: number[] = [];
        yearCompresseurK245Data.dates.forEach((date) => {
          const dayRows = yearCompresseurK245Data.rowsByDate[date] ?? [];
          const parts = date.split('-').map(Number);
          const year = parts[0];
          const month = parts[1];
          const day = parts[2];
    ...
    const categories = COMPRESSEUR_K245_HOURS.map((h) => compresseurK245HourLabels[h]);
    const values = COMPRESSEUR_K245_HOURS.map((h, i) => {
      const row = data[i];
  ...
  const options: ApexOptions = useMemo(() => {
    const hourColorsArr = COMPRESSEUR_K245_HOURS.map((h) => HOUR_COLORS[h as keyof typeof HOUR_COLORS]);
...
export default ChartCompresseurK245;
