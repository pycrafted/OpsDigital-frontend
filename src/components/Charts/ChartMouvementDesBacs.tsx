...
import type { HourRow } from '../../data/mouvementDesBacs';
import {
...
const indicateurOptions = getMouvementBacsIndicateurOptions();
...
/** Données par date (semaine / mois / trimestre / semestre / année). */
export interface WeekMouvementBacsData {
  dates: string[];
  rowsByDate: Record<string, HourRow[]>;
}
export interface MonthMouvementBacsData {
  dates: string[];
  rowsByDate: Record<string, HourRow[]>;
}
export interface QuarterMouvementBacsData {
  dates: string[];
  rowsByDate: Record<string, HourRow[]>;
}
export interface SemesterMouvementBacsData {
  dates: string[];
  rowsByDate: Record<string, HourRow[]>;
}
export interface YearMouvementBacsData {
  dates: string[];
  rowsByDate: Record<string, HourRow[]>;
}

export interface ChartMouvementDesBacsProps {
  data: HourRow[];
  ...
  weekMouvementBacsData?: WeekMouvementBacsData;
  monthMouvementBacsData?: MonthMouvementBacsData;
  quarterMouvementBacsData?: QuarterMouvementBacsData;
  semesterMouvementBacsData?: SemesterMouvementBacsData;
  yearMouvementBacsData?: YearMouvementBacsData;
  embedded?: boolean;
...
/** Trimestre = continuité de la courbe mensuelle : mois 1 + mois 2 + mois 3 collés (3× plus de relevés). */
function getExampleQuarterData(selectedQuarter?: string): { categories: string[]; series: { name: string; data: number[] }[] } {
  let year: number;
  ...
  for (let m = 0; m < 3; m++) {
    ...
    const monthResult = getExampleMonthData(monthStr);
...
/** Données vides pour le trimestre (même structure que getEmptySemesterData, 3 mois). */
...
function getEmptyWeekData(): { categories: string[]; series: { name: string; data: number[] }[] } {
  ...
  const hourSuffixes = [...MOUVEMENT_BACS_HOURS];
...
const MOUVEMENT_BACS_HOURS_PER_DAY = [...MOUVEMENT_BACS_HOURS];
...
function getEmptyMonthData(selectedMonth?: string): { categories: string[]; series: { name: string; data: number[] }[] } {
  ...
  for (let dayIndex = 0; dayIndex < daysInMonth; dayIndex++) {
    ...
    MOUVEMENT_BACS_HOURS_PER_DAY.forEach((hourLabel) => {
...
const ChartMouvementDesBacs: React.FC<ChartMouvementDesBacsProps> = ({
  data,
  duration,
  ...
  selectedIndicateur,
  onIndicateurChange,
  weekMouvementBacsData,
  monthMouvementBacsData,
  quarterMouvementBacsData,
  semesterMouvementBacsData,
  yearMouvementBacsData,
  embedded = false,
  leftSlot,
  centerSlot,
  rightSlot,
  ...
  const indicateurLabel = getFieldLabel('mouvement-des-bacs', selectedIndicateur, indicateurOptions.find((o) => o.key === selectedIndicateur)?.label ?? selectedIndicateur);

  const chartData = useMemo(() => {
    if (duration === 'week') {
      if (weekMouvementBacsData && weekMouvementBacsData.dates.length > 0) {
        const categories: string[] = [];
        const chartValues: number[] = [];
        const outOfBoundsIndices: number[] = [];
        const conformeIndices: number[] = [];
        weekMouvementBacsData.dates.forEach((date, dayIndex) => {
          const dayRows = weekMouvementBacsData.rowsByDate[date] ?? [];
          MOUVEMENT_BACS_HOURS.forEach((h) => {
            categories.push(`${WEEK_DAY_LABELS[dayIndex]} ${h}`);
    ...
    if (duration === 'month') {
      if (monthMouvementBacsData && monthMouvementBacsData.dates.length > 0) {
        const categories: string[] = [];
        const chartValues: number[] = [];
        const outOfBoundsIndices: number[] = [];
        const conformeIndices: number[] = [];
        monthMouvementBacsData.dates.forEach((dateStr) => {
          const [y, m, d] = dateStr.split('-').map(Number);
          const dateLabel = Number.isFinite(y) && Number.isFinite(m) && Number.isFinite(d)
            ? formatDateDDMMYYYY(d, m, y)
            : dateStr;
          const dayRows = monthMouvementBacsData.rowsByDate[dateStr] ?? [];
          MOUVEMENT_BACS_HOURS.forEach((h) => {
    ...
    if (duration === 'quarter') {
      if (quarterMouvementBacsData && quarterMouvementBacsData.dates.length > 0) {
        const categories: string[] = [];
        const chartValues: number[] = [];
        quarterMouvementBacsData.dates.forEach((dateStr) => {
          const [y, m, d] = dateStr.split('-').map(Number);
          const dateLabel = Number.isFinite(y) && Number.isFinite(m) && Number.isFinite(d)
            ? formatDateDDMMYYYY(d, m, y)
            : dateStr;
          const dayRows = quarterMouvementBacsData.rowsByDate[dateStr] ?? [];
          MOUVEMENT_BACS_HOURS.forEach((h) => {
            categories.push(`${dateLabel} ${h}`);
            const row = dayRows.find((r) => r.hour === h);
    ...
    if (duration === 'semester') {
      if (semesterMouvementBacsData && semesterMouvementBacsData.dates.length > 0) {
        const categories: string[] = [];
        const chartValues: number[] = [];
        semesterMouvementBacsData.dates.forEach((dateStr) => {
          const [y, m, d] = dateStr.split('-').map(Number);
          const dateLabel = Number.isFinite(y) && Number.isFinite(m) && Number.isFinite(d)
            ? formatDateDDMMYYYY(d, m, y)
            : dateStr;
          const dayRows = semesterMouvementBacsData.rowsByDate[dateStr] ?? [];
          MOUVEMENT_BACS_HOURS.forEach((h) => {
            categories.push(`${dateLabel} ${h}`);
            const row = dayRows.find((r) => r.hour === h);
            const v = row ? parseValue(row.values[selectedIndicateur] ?? '') : 0;
    ...
    if (duration === 'year') {
      if (yearMouvementBacsData && yearMouvementBacsData.dates.length > 0) {
        const categories: string[] = [];
        const chartValues: number[] = [];
        yearMouvementBacsData.dates.forEach((dateStr) => {
          const [y, m, d] = dateStr.split('-').map(Number);
          const dateLabel = Number.isFinite(y) && Number.isFinite(m) && Number.isFinite(d)
            ? formatDateDDMMYYYY(d, m, y)
            : dateStr;
          const dayRows = yearMouvementBacsData.rowsByDate[dateStr] ?? [];
          MOUVEMENT_BACS_HOURS.forEach((h) => {
            categories.push(`${dateLabel} ${h}`);
            const row = dayRows.find((r) => r.hour === h);
            const v = row ? parseValue(row.values[selectedIndicateur] ?? '') : 0;
    ...
    const categories = [...MOUVEMENT_BACS_HOURS];
    const values = MOUVEMENT_BACS_HOURS.map((h, i) => {
      const row = data[i];
  ...
  }, [data, duration, selectedIndicateur, indicateurLabel, selectedMonthProp, selectedQuarterProp, selectedSemesterProp, selectedYearProp, weekMouvementBacsData, monthMouvementBacsData, quarterMouvementBacsData, semesterMouvementBacsData, yearMouvementBacsData, isOutOfBounds]);

  const isDarkMode = colorMode === 'dark';

  const options: ApexOptions = useMemo(() => {
    const hourColorsArr = MOUVEMENT_BACS_HOURS.map((h) => HOUR_COLORS[h] ?? '#3C50E0');
      ...
      <div className="w-full min-h-[300px]">
        {chartData.series.length > 0 && chartData.categories.length > 0 ? (
          <ReactApexChart
            ...
            className={isWeek ? 'mouvement-bacs-week-chart' : undefined}
            type={isDay ? 'bar' : 'area'}
            height={350}
          />
        ) : (
...
export default ChartMouvementDesBacs;
