...
const TableCompresseurK244 = ({
  ...
  const { getHourLabel, getCategoryLabel, getMeasureLabel } = useCompresseurK244Labels();
  ...
  return (
    <div className="relative flex min-h-0 min-w-0 flex-1 flex-col gap-6">
      ...
      {/* Tableau — directement sur la page, sans card */}
                ...
                {visibleCategories.flatMap((cat) =>
                  cat.subRows.map((subRow) => {
                    ...
                    return (
                      ...
                      >
                        <span className="block truncate" title={getMeasureLabel(mKey)}>{getMeasureLabel(mKey)}</span>
                      </th>
                    );
                  })
                )}
