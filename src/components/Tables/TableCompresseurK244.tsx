...
import {
  COMPRESSEUR_K244_CATEGORIES as categories,
...
const FEUILLE_ID = "compresseur-k244";

const hours = [...COMPRESSEUR_K244_HOURS];
...
export interface TableCompresseurK244Props {
  data: HourRow[];
...
const TableCompresseurK244 = ({
...
}: TableCompresseurK244Props) => {
  const { getHourLabel } = useCompresseurK244Labels();
  const { getCategoryLabel: _getCat, getFieldLabel } = useRenommage();
  const getCategoryLabel = (cat: string) => _getCat('compresseur-k244', cat);
  const getMeasureLabel = (key: string, defaultLabel?: string) => getFieldLabel('compresseur-k244', key, defaultLabel ?? key);
  ...
  categories.forEach((c) => { defaultSubRowOrders[c.category] = [...c.subRows]; });
  ...
  React.useEffect(() => {
    fetchTableSettings(FEUILLE_ID).then((s) => {
      if (Array.isArray(s["column_order"])) setCategoryOrder(s["column_order"] as string[]);
        ...
        try { const v = localStorage.getItem("k244_column_order"); if (v) setCategoryOrder(JSON.parse(v) as string[]); } catch (_) {}
      }
      if (s["subrow_orders"] && typeof s["subrow_orders"] === "object" && !Array.isArray(s["subrow_orders"])) {
        ...
        try { const v = localStorage.getItem("k244_subrow_orders"); if (v) setSubRowOrders(JSON.parse(v) as Record<string, string[]>); } catch (_) {}
      }
    }).catch((_) => {
      try { const v = localStorage.getItem("k244_column_order"); if (v) setCategoryOrder(JSON.parse(v) as string[]); } catch (_2) {}
      try { const v = localStorage.getItem("k244_subrow_orders"); if (v) setSubRowOrders(JSON.parse(v) as Record<string, string[]>); } catch (_2) {}
    });
  }, []);
  ...
  function scheduleSave(colOrder: string[], subOrders: Record<string, string[]>) {
    ...
    saveTimerRef.current = setTimeout(() => {
      localStorage.setItem("k244_column_order", JSON.stringify(colOrder));
      localStorage.setItem("k244_subrow_orders", JSON.stringify(subOrders));
  ...
  function handleSubDrop(e: React.DragEvent<HTMLTableCellElement>, cat: string, toSub: string) {
    ...
    const side = getSide(e);
    const base = subRowOrders[cat] ?? categories.find((c) => c.category === cat)?.subRows ?? [];
    const arr = [...base];
  ...
  const currentCategories = filteredCategories
    .map((cat) => {
      const orderedSubs = (subRowOrders[cat.category] ?? cat.subRows).filter((s) => cat.subRows.includes(s) && selectedSubRows.includes(s));
                ...
                {visibleCategories.flatMap((cat) =>
                  cat.subRows.map((subRow) => {
                    ...
                    return (
                      ...
                      >
                        <span className="block truncate" title={getMeasureLabel(mKey, subRow)}>{getMeasureLabel(mKey, subRow)}</span>
                      </th>
                    );
                  })
                )}
                  ...
                  return (
                    ...
                    >
                      <td className={`sticky left-0 z-10 w-28 min-w-[6.5rem] max-w-[7rem] border-r border-stroke/70 bg-[#3c50e0] py-1 pl-2 pr-2 text-sm font-medium text-white dark:border-strokedark dark:bg-[#3c50e0] dark:text-white ${canEdit ? 'group-hover:bg-[#3c50e0]/90 dark:group-hover:bg-[#3c50e0]/90' : ''}`}>
                        <span className="block truncate" title={getHourLabel(hourRow.hour)}>{getHourLabel(hourRow.hour)}</span>
                      </td>
                      {visibleCategories.flatMap((cat, catIndex) =>
                        cat.subRows.map((subRow, subIndex) => {
                          ...
                          return (
                            <td
                              key={key}
                              className={`w-[7rem] min-w-[7rem] max-w-[7rem] border-r border-stroke/50 py-0 px-1 dark:border-strokedark/70 ${
