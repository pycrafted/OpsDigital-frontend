...
const FEUILLE_ID = "compresseur-k245";
...
const TableCompresseurK245 = ({
  ...
  const { getHourLabel } = useCompresseurK245Labels();
  const { getCategoryLabel: _getCat, getFieldLabel } = useRenommage();
  const getCategoryLabel = (cat: string) => _getCat('compresseur-k245', cat);
  const getMeasureLabel = (key: string, defaultLabel?: string) => getFieldLabel('compresseur-k245', key, defaultLabel ?? key);
  ...
  React.useEffect(() => {
    fetchTableSettings(FEUILLE_ID).then((s) => {
      if (Array.isArray(s["column_order"])) setCategoryOrder(s["column_order"] as string[]);
        ...
        try { const v = localStorage.getItem("k245_column_order"); if (v) setCategoryOrder(JSON.parse(v) as string[]); } catch (_) {}
      }
      if (s["subrow_orders"] && typeof s["subrow_orders"] === "object" && !Array.isArray(s["subrow_orders"])) {
        ...
        try { const v = localStorage.getItem("k245_subrow_orders"); if (v) setSubRowOrders(JSON.parse(v) as Record<string, string[]>); } catch (_) {}
      }
    }).catch((_) => {
      try { const v = localStorage.getItem("k245_column_order"); if (v) setCategoryOrder(JSON.parse(v) as string[]); } catch (_2) {}
      try { const v = localStorage.getItem("k245_subrow_orders"); if (v) setSubRowOrders(JSON.parse(v) as Record<string, string[]>); } catch (_2) {}
    });
  }, []);
  ...
  function scheduleSave(colOrder: string[], subOrders: Record<string, string[]>) {
    ...
    saveTimerRef.current = setTimeout(() => {
      localStorage.setItem("k245_column_order", JSON.stringify(colOrder));
      localStorage.setItem("k245_subrow_orders", JSON.stringify(subOrders));
              ...
              <tr>
                {visibleCategories.flatMap((cat) =>
                  cat.subRows.map((subRow) => {
                    ...
                    const label = getMeasureLabel(mKey, subRow);
