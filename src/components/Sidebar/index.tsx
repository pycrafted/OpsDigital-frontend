import React, { useEffect, useRef, useState } from 'react';
import { NavLink, useLocation } from 'react-router-dom';
import SidebarLinkGroup from './SidebarLinkGroup';
import Logo from '../../images/logo/logo.svg';

interface SidebarProps {
  sidebarOpen: boolean;
  setSidebarOpen: (arg: boolean) => void;
}

const Sidebar = ({ sidebarOpen, setSidebarOpen }: SidebarProps) => {
  const location = useLocation();
  const { pathname } = location;

  const trigger = useRef<any>(null);
  const sidebar = useRef<any>(null);

  const sidebarExpanded = true;

  // close on click outside
  useEffect(() => {
    const clickHandler = ({ target }: MouseEvent) => {
      if (!sidebar.current || !trigger.current) return;
      if (
        !sidebarOpen ||
        sidebar.current.contains(target) ||
        trigger.current.contains(target)
      )
        return;
      setSidebarOpen(false);
    };
    document.addEventListener('click', clickHandler);
    return () => document.removeEventListener('click', clickHandler);
  });

  // close if the esc key is pressed
  useEffect(() => {
    const keyHandler = ({ keyCode }: KeyboardEvent) => {
      if (!sidebarOpen || keyCode !== 27) return;
      setSidebarOpen(false);
    };
    document.addEventListener('keydown', keyHandler);
    return () => document.removeEventListener('keydown', keyHandler);
  });

  useEffect(() => {
    document.querySelector('body')?.classList.add('sidebar-expanded');
  }, []);

  return (
    <aside
      ref={sidebar}
      className={`absolute left-0 top-0 z-9999 flex h-screen flex-col overflow-y-hidden bg-black duration-300 ease-linear dark:bg-boxdark lg:static lg:translate-x-0 ${
        sidebarOpen ? 'translate-x-0' : '-translate-x-full'
      } ${
        sidebarExpanded ? 'w-60' : 'w-16'
      }`}
    >
      {/* <!-- SIDEBAR HEADER --> */}
      <div className="flex items-center justify-between gap-2 px-4 py-3 lg:py-4">
        {sidebarExpanded && (
          <NavLink to="/">
            <img src={Logo} alt="Logo" className="h-6" />
          </NavLink>
        )}

        <div className="flex items-center gap-2">

          {/* Close button for mobile */}
          <button
            ref={trigger}
            onClick={() => setSidebarOpen(!sidebarOpen)}
            aria-controls="sidebar"
            aria-expanded={sidebarOpen}
            className="block lg:hidden"
          >
            <svg
              className="fill-current"
              width="20"
              height="18"
              viewBox="0 0 20 18"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M19 8.175H2.98748L9.36248 1.6875C9.69998 1.35 9.69998 0.825 9.36248 0.4875C9.02498 0.15 8.49998 0.15 8.16248 0.4875L0.399976 8.3625C0.0624756 8.7 0.0624756 9.225 0.399976 9.5625L8.16248 17.4375C8.31248 17.5875 8.53748 17.7 8.76248 17.7C8.98748 17.7 9.17498 17.625 9.36248 17.475C9.69998 17.1375 9.69998 16.6125 9.36248 16.275L3.02498 9.8625H19C19.45 9.8625 19.825 9.4875 19.825 9.0375C19.825 8.55 19.45 8.175 19 8.175Z"
                fill=""
              />
            </svg>
          </button>
        </div>
      </div>
      {/* <!-- SIDEBAR HEADER --> */}

      <div className="no-scrollbar flex flex-col overflow-y-auto duration-300 ease-linear">
        {/* <!-- Sidebar Menu --> */}
        <nav className={`mt-3 py-3 lg:mt-6 ${sidebarExpanded ? 'px-3 lg:px-4' : 'px-2'}`}>
          {/* <!-- Menu Group --> */}
          <div>
            <h3 className={`mb-2 text-xs font-semibold text-bodydark2 transition-opacity duration-300 ${
              sidebarExpanded ? 'ml-4 opacity-100' : 'opacity-0 h-0 mb-0 overflow-hidden'
            }`}>
              MENU
            </h3>

            <ul className="mb-4 flex flex-col gap-1">
              {/* <!-- Menu Item Tableau de bord --> */}
              <SidebarLinkGroup
                activeCondition={
                  pathname === '/tableau-de-bord'
                }
              >
                {(handleClick, open) => {
                  return (
                    <React.Fragment>
                      <NavLink
                        to="#"
                        className={`group relative flex items-center rounded-sm py-1.5 text-xs font-semibold text-bodydark1 duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4 ${
                          sidebarExpanded ? 'gap-2 px-3' : 'justify-center px-2'
                        } ${
                          pathname === '/tableau-de-bord' &&
                          'bg-graydark dark:bg-meta-4'
                        }`}
                        onClick={(e) => {
                          e.preventDefault();
                          handleClick();
                        }}
                      >
                        <svg
                          className="fill-current"
                          width="14"
                          height="14"
                          viewBox="0 0 18 18"
                          fill="none"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <path
                            d="M6.10322 0.956299H2.53135C1.5751 0.956299 0.787598 1.7438 0.787598 2.70005V6.27192C0.787598 7.22817 1.5751 8.01567 2.53135 8.01567H6.10322C7.05947 8.01567 7.84697 7.22817 7.84697 6.27192V2.72817C7.8751 1.7438 7.0876 0.956299 6.10322 0.956299ZM6.60947 6.30005C6.60947 6.5813 6.38447 6.8063 6.10322 6.8063H2.53135C2.2501 6.8063 2.0251 6.5813 2.0251 6.30005V2.72817C2.0251 2.44692 2.2501 2.22192 2.53135 2.22192H6.10322C6.38447 2.22192 6.60947 2.44692 6.60947 2.72817V6.30005Z"
                            fill=""
                          />
                          <path
                            d="M15.4689 0.956299H11.8971C10.9408 0.956299 10.1533 1.7438 10.1533 2.70005V6.27192C10.1533 7.22817 10.9408 8.01567 11.8971 8.01567H15.4689C16.4252 8.01567 17.2127 7.22817 17.2127 6.27192V2.72817C17.2127 1.7438 16.4252 0.956299 15.4689 0.956299ZM15.9752 6.30005C15.9752 6.5813 15.7502 6.8063 15.4689 6.8063H11.8971C11.6158 6.8063 11.3908 6.5813 11.3908 6.30005V2.72817C11.3908 2.44692 11.6158 2.22192 11.8971 2.22192H15.4689C15.7502 2.22192 15.9752 2.44692 15.9752 2.72817V6.30005Z"
                            fill=""
                          />
                          <path
                            d="M6.10322 9.92822H2.53135C1.5751 9.92822 0.787598 10.7157 0.787598 11.672V15.2438C0.787598 16.2001 1.5751 16.9876 2.53135 16.9876H6.10322C7.05947 16.9876 7.84697 16.2001 7.84697 15.2438V11.7001C7.8751 10.7157 7.0876 9.92822 6.10322 9.92822ZM6.60947 15.272C6.60947 15.5532 6.38447 15.7782 6.10322 15.7782H2.53135C2.2501 15.7782 2.0251 15.5532 2.0251 15.272V11.7001C2.0251 11.4188 2.2501 11.1938 2.53135 11.1938H6.10322C6.38447 11.1938 6.60947 11.4188 6.60947 11.7001V15.272Z"
                            fill=""
                          />
                          <path
                            d="M15.4689 9.92822H11.8971C10.9408 9.92822 10.1533 10.7157 10.1533 11.672V15.2438C10.1533 16.2001 10.9408 16.9876 11.8971 16.9876H15.4689C16.4252 16.9876 17.2127 16.2001 17.2127 15.2438V11.7001C17.2127 10.7157 16.4252 9.92822 15.4689 9.92822ZM15.9752 15.272C15.9752 15.5532 15.7502 15.7782 15.4689 15.7782H11.8971C11.6158 15.7782 11.3908 15.5532 11.3908 15.272V11.7001C11.3908 11.4188 11.6158 11.1938 11.8971 11.1938H15.4689C15.7502 11.1938 15.9752 11.4188 15.9752 11.7001V15.272Z"
                            fill=""
                          />
                        </svg>
                        <span className={`transition-opacity duration-300 ${
                          sidebarExpanded ? 'opacity-100' : 'opacity-0 w-0 overflow-hidden'
                        }`}>
                          Tableau de bord
                        </span>
                        <svg
                          className={`fill-current transition-opacity duration-300 ${
                            sidebarExpanded ? 'opacity-100 absolute right-4 top-1/2 -translate-y-1/2' : 'opacity-0 w-0 overflow-hidden'
                          } ${open && 'rotate-180'}`}
                          width="14"
                          height="14"
                          viewBox="0 0 20 20"
                          fill="none"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <path
                            fillRule="evenodd"
                            clipRule="evenodd"
                            d="M4.41107 6.9107C4.73651 6.58527 5.26414 6.58527 5.58958 6.9107L10.0003 11.3214L14.4111 6.91071C14.7365 6.58527 15.2641 6.58527 15.5896 6.91071C15.915 7.23614 15.915 7.76378 15.5896 8.08922L10.5896 13.0892C10.2641 13.4147 9.73651 13.4147 9.41107 13.0892L4.41107 8.08922C4.08563 7.76378 4.08563 7.23614 4.41107 6.9107Z"
                            fill=""
                          />
                        </svg>
                      </NavLink>
                      <div
                        className={`translate transform overflow-hidden ${
                          (!open || !sidebarExpanded) && 'hidden'
                        }`}
                      >
                        <ul className={`mt-2 mb-3 flex flex-col gap-1.5 ${sidebarExpanded ? 'pl-5' : 'pl-2'}`}>
                          <li>
                            <NavLink
                              to="/tableau-de-bord"
                              className={({ isActive }) =>
                                `group relative flex items-center rounded-md text-xs font-semibold text-bodydark2 duration-300 ease-in-out hover:text-white ${
                                  sidebarExpanded ? 'gap-2 px-3' : 'justify-center px-2'
                                } ` +
                                (isActive && '!text-white')
                              }
                            >
                              <span className={`transition-opacity duration-300 ${
                                sidebarExpanded ? 'opacity-100' : 'opacity-0 w-0 overflow-hidden'
                              }`}>
                                eCommerce
                              </span>
                            </NavLink>
                          </li>
                        </ul>
                      </div>
                    </React.Fragment>
                  );
                }}
              </SidebarLinkGroup>
              {/* <!-- Menu Item Tableau de bord --> */}

              {/* <!-- Menu Item Analyses du laboratoire --> */}
              <li>
                <NavLink
                  to="/analyses-laboratoire"
                  className={`group relative flex items-center rounded-sm py-1.5 text-xs font-semibold text-bodydark1 duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4 ${
                    sidebarExpanded ? 'gap-2 px-3' : 'justify-center px-2'
                  } ${
                    pathname.includes('analyses-laboratoire') &&
                    'bg-graydark dark:bg-meta-4'
                  }`}
                >
                  <svg
                    className="fill-current"
                    width="14"
                    height="14"
                    viewBox="0 0 18 18"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M9 0.75C4.725 0.75 1.25 4.225 1.25 8.5C1.25 12.775 4.725 16.25 9 16.25C13.275 16.25 16.75 12.775 16.75 8.5C16.75 4.225 13.275 0.75 9 0.75ZM9 15C5.45 15 2.5 12.05 2.5 8.5C2.5 4.95 5.45 2 9 2C12.55 2 15.5 4.95 15.5 8.5C15.5 12.05 12.55 15 9 15Z"
                      fill=""
                    />
                    <path
                      d="M9 4.5C8.585 4.5 8.25 4.835 8.25 5.25V8.5C8.25 8.915 8.585 9.25 9 9.25C9.415 9.25 9.75 8.915 9.75 8.5V5.25C9.75 4.835 9.415 4.5 9 4.5Z"
                      fill=""
                    />
                    <path
                      d="M9 11.5C9.28 11.5 9.5 11.72 9.5 12C9.5 12.28 9.28 12.5 9 12.5C8.72 12.5 8.5 12.28 8.5 12C8.5 11.72 8.72 11.5 9 11.5Z"
                      fill=""
                    />
                    <path
                      d="M6.5 3.5L5.75 4.25L4.25 2.75L5 2C5.39 1.61 6.02 1.61 6.41 2C6.8 2.39 6.8 3.02 6.41 3.41L6.5 3.5Z"
                      fill=""
                    />
                    <path
                      d="M11.5 3.5L12.25 4.25L13.75 2.75L13 2C12.61 1.61 11.98 1.61 11.59 2C11.2 2.39 11.2 3.02 11.59 3.41L11.5 3.5Z"
                      fill=""
                    />
                    <path
                      d="M3.5 6.5L4.25 5.75L2.75 4.25L2 5C1.61 5.39 1.61 6.02 2 6.41C2.39 6.8 3.02 6.8 3.41 6.41L3.5 6.5Z"
                      fill=""
                    />
                    <path
                      d="M14.5 6.5L13.75 5.75L15.25 4.25L16 5C16.39 5.39 16.39 6.02 16 6.41C15.61 6.8 14.98 6.8 14.59 6.41L14.5 6.5Z"
                      fill=""
                    />
                  </svg>
                  <span className={`transition-opacity duration-300 ${
                    sidebarExpanded ? 'opacity-100' : 'opacity-0 w-0 overflow-hidden'
                  }`}>
                    Analyses du laboratoire
                  </span>
                </NavLink>
              </li>
              {/* <!-- Menu Item Analyses du laboratoire --> */}

              {/* <!-- Menu Item Réformateur catalytique --> */}
              <li>
                <NavLink
                  to="/reformateur-catalytique"
                  className={`group relative flex items-center rounded-sm py-1.5 text-xs font-semibold text-bodydark1 duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4 ${
                    sidebarExpanded ? 'gap-2 px-3' : 'justify-center px-2'
                  } ${
                    pathname.includes('reformateur-catalytique') &&
                    'bg-graydark dark:bg-meta-4'
                  }`}
                >
                  <svg
                    className="fill-current"
                    width="14"
                    height="14"
                    viewBox="0 0 18 18"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M5.25 2.25H12.75C13.1625 2.25 13.5 2.5875 13.5 3V15C13.5 15.4125 13.1625 15.75 12.75 15.75H5.25C4.8375 15.75 4.5 15.4125 4.5 15V3C4.5 2.5875 4.8375 2.25 5.25 2.25ZM6 4.5V14.25H12V4.5H6Z"
                      fill=""
                    />
                    <path
                      d="M3 6.75H15V7.5H3V6.75Z"
                      fill=""
                    />
                    <path
                      d="M3 10.5H15V11.25H3V10.5Z"
                      fill=""
                    />
                    <path
                      d="M7.5 1.5V3H10.5V1.5H7.5Z"
                      fill=""
                    />
                    <path
                      d="M7.5 15V16.5H10.5V15H7.5Z"
                      fill=""
                    />
                  </svg>
                  <span className={`transition-opacity duration-300 ${
                    sidebarExpanded ? 'opacity-100' : 'opacity-0 w-0 overflow-hidden'
                  }`}>
                    Réformateur catalytique
                  </span>
                </NavLink>
              </li>
              {/* <!-- Menu Item Réformateur catalytique --> */}

              {/* <!-- Menu Item Mouvement des bacs --> */}
              <li>
                <NavLink
                  to="/mouvement-des-bacs"
                  className={`group relative flex items-center rounded-sm py-1.5 text-xs font-semibold text-bodydark1 duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4 ${
                    sidebarExpanded ? 'gap-2 px-3' : 'justify-center px-2'
                  } ${
                    pathname.includes('mouvement-des-bacs') &&
                    'bg-graydark dark:bg-meta-4'
                  }`}
                >
                  <svg
                    className="fill-current"
                    width="14"
                    height="14"
                    viewBox="0 0 18 18"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M3 2.25H15C15.414 2.25 15.75 2.586 15.75 3V15C15.75 15.414 15.414 15.75 15 15.75H3C2.586 15.75 2.25 15.414 2.25 15V3C2.25 2.586 2.586 2.25 3 2.25ZM3.75 3.75V14.25H14.25V3.75H3.75Z"
                      fill=""
                    />
                    <path
                      d="M3 5.25H15V6H3V5.25Z"
                      fill=""
                    />
                    <path
                      d="M3 8.25H15V9H3V8.25Z"
                      fill=""
                    />
                    <path
                      d="M3 11.25H15V12H3V11.25Z"
                      fill=""
                    />
                    <path
                      d="M6 1.5V2.25H12V1.5H6Z"
                      fill=""
                    />
                    <path
                      d="M6 15.75V16.5H12V15.75H6Z"
                      fill=""
                    />
                  </svg>
                  <span className={`transition-opacity duration-300 ${
                    sidebarExpanded ? 'opacity-100' : 'opacity-0 w-0 overflow-hidden'
                  }`}>
                    mouvement des bacs
                  </span>
                </NavLink>
              </li>
              {/* <!-- Menu Item Mouvement des bacs --> */}

              {/* <!-- Menu Item Production : valeur/electricité --> */}
              <li>
                <NavLink
                  to="/production-valeur-electricite"
                  className={`group relative flex items-center rounded-sm py-1.5 text-xs font-semibold text-bodydark1 duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4 ${
                    sidebarExpanded ? 'gap-2 px-3' : 'justify-center px-2'
                  } ${
                    pathname.includes('production-valeur-electricite') &&
                    'bg-graydark dark:bg-meta-4'
                  }`}
                >
                  <svg
                    className="fill-current"
                    width="14"
                    height="14"
                    viewBox="0 0 18 18"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M9 0L10.545 6.545L17.09 8L10.545 9.455L9 16L7.455 9.455L0.91 8L7.455 6.545L9 0Z"
                      fill=""
                    />
                    <path
                      d="M9 2.5L9.5 5.5L12.5 6L9.5 6.5L9 9.5L8.5 6.5L5.5 6L8.5 5.5L9 2.5Z"
                      fill=""
                    />
                    <path
                      d="M6 3L6.5 5L8.5 5.5L6.5 6L6 8L5.5 6L3.5 5.5L5.5 5L6 3Z"
                      fill=""
                    />
                    <path
                      d="M12 3L12.5 5L14.5 5.5L12.5 6L12 8L11.5 6L9.5 5.5L11.5 5L12 3Z"
                      fill=""
                    />
                    <path
                      d="M9 11L9.3 12.3L10.6 12.6L9.3 12.9L9 14.2L8.7 12.9L7.4 12.6L8.7 12.3L9 11Z"
                      fill=""
                    />
                  </svg>
                  <span className={`transition-opacity duration-300 ${
                    sidebarExpanded ? 'opacity-100' : 'opacity-0 w-0 overflow-hidden'
                  }`}>
                    valeur/electricité
                  </span>
                </NavLink>
              </li>
              {/* <!-- Menu Item Production : valeur/electricité --> */}

              {/* <!-- Menu Item Compresseur K 245 --> */}
              <li>
                <NavLink
                  to="/compresseur-k245"
                  className={`group relative flex items-center rounded-sm py-1.5 text-xs font-semibold text-bodydark1 duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4 ${
                    sidebarExpanded ? 'gap-2 px-3' : 'justify-center px-2'
                  } ${
                    pathname.includes('compresseur-k245') &&
                    'bg-graydark dark:bg-meta-4'
                  }`}
                >
                  <svg
                    className="fill-current"
                    width="14"
                    height="14"
                    viewBox="0 0 18 18"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M9 0.75C4.725 0.75 1.25 4.225 1.25 8.5C1.25 12.775 4.725 16.25 9 16.25C13.275 16.25 16.75 12.775 16.75 8.5C16.75 4.225 13.275 0.75 9 0.75ZM9 15C5.45 15 2.5 12.05 2.5 8.5C2.5 4.95 5.45 2 9 2C12.55 2 15.5 4.95 15.5 8.5C15.5 12.05 12.55 15 9 15Z"
                      fill=""
                    />
                    <path
                      d="M9 3.75C8.17 3.75 7.5 4.42 7.5 5.25V7.5C7.5 8.33 8.17 9 9 9C9.83 9 10.5 8.33 10.5 7.5V5.25C10.5 4.42 9.83 3.75 9 3.75Z"
                      fill=""
                    />
                    <path
                      d="M9 10.5C9.28 10.5 9.5 10.72 9.5 11C9.5 11.28 9.28 11.5 9 11.5C8.72 11.5 8.5 11.28 8.5 11C8.5 10.72 8.72 10.5 9 10.5Z"
                      fill=""
                    />
                    <path
                      d="M12.5 4.5L11.75 5.25L10.25 3.75L11 3C11.39 2.61 12.02 2.61 12.41 3C12.8 3.39 12.8 4.02 12.41 4.41L12.5 4.5Z"
                      fill=""
                    />
                    <path
                      d="M5.5 4.5L6.25 5.25L7.75 3.75L7 3C6.61 2.61 5.98 2.61 5.59 3C5.2 3.39 5.2 4.02 5.59 4.41L5.5 4.5Z"
                      fill=""
                    />
                    <path
                      d="M4.5 7.5L5.25 6.75L3.75 5.25L3 6C2.61 6.39 2.61 7.02 3 7.41C3.39 7.8 4.02 7.8 4.41 7.41L4.5 7.5Z"
                      fill=""
                    />
                    <path
                      d="M13.5 7.5L12.75 6.75L14.25 5.25L15 6C15.39 6.39 15.39 7.02 15 7.41C14.61 7.8 13.98 7.8 13.59 7.41L13.5 7.5Z"
                      fill=""
                    />
                    <path
                      d="M6 12L7 11L8 12L7 13L6 12Z"
                      fill=""
                    />
                    <path
                      d="M10 12L11 11L12 12L11 13L10 12Z"
                      fill=""
                    />
                  </svg>
                  <span className={`transition-opacity duration-300 ${
                    sidebarExpanded ? 'opacity-100' : 'opacity-0 w-0 overflow-hidden'
                  }`}>
                    Compresseur K 245
                  </span>
                </NavLink>
              </li>
              {/* <!-- Menu Item Compresseur K 245 --> */}

              {/* <!-- Menu Item Compresseur k244 --> */}
              <li>
                <NavLink
                  to="/compresseur-k244"
                  className={`group relative flex items-center rounded-sm py-1.5 text-xs font-semibold text-bodydark1 duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4 ${
                    sidebarExpanded ? 'gap-2 px-3' : 'justify-center px-2'
                  } ${
                    pathname.includes('compresseur-k244') &&
                    'bg-graydark dark:bg-meta-4'
                  }`}
                >
                  <svg
                    className="fill-current"
                    width="14"
                    height="14"
                    viewBox="0 0 18 18"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M9 0.75C4.725 0.75 1.25 4.225 1.25 8.5C1.25 12.775 4.725 16.25 9 16.25C13.275 16.25 16.75 12.775 16.75 8.5C16.75 4.225 13.275 0.75 9 0.75ZM9 15C5.45 15 2.5 12.05 2.5 8.5C2.5 4.95 5.45 2 9 2C12.55 2 15.5 4.95 15.5 8.5C15.5 12.05 12.55 15 9 15Z"
                      fill=""
                    />
                    <path
                      d="M9 3.75C8.17 3.75 7.5 4.42 7.5 5.25V7.5C7.5 8.33 8.17 9 9 9C9.83 9 10.5 8.33 10.5 7.5V5.25C10.5 4.42 9.83 3.75 9 3.75Z"
                      fill=""
                    />
                    <path
                      d="M9 10.5C9.28 10.5 9.5 10.72 9.5 11C9.5 11.28 9.28 11.5 9 11.5C8.72 11.5 8.5 11.28 8.5 11C8.5 10.72 8.72 10.5 9 10.5Z"
                      fill=""
                    />
                    <path
                      d="M12.5 4.5L11.75 5.25L10.25 3.75L11 3C11.39 2.61 12.02 2.61 12.41 3C12.8 3.39 12.8 4.02 12.41 4.41L12.5 4.5Z"
                      fill=""
                    />
                    <path
                      d="M5.5 4.5L6.25 5.25L7.75 3.75L7 3C6.61 2.61 5.98 2.61 5.59 3C5.2 3.39 5.2 4.02 5.59 4.41L5.5 4.5Z"
                      fill=""
                    />
                    <path
                      d="M4.5 7.5L5.25 6.75L3.75 5.25L3 6C2.61 6.39 2.61 7.02 3 7.41C3.39 7.8 4.02 7.8 4.41 7.41L4.5 7.5Z"
                      fill=""
                    />
                    <path
                      d="M13.5 7.5L12.75 6.75L14.25 5.25L15 6C15.39 6.39 15.39 7.02 15 7.41C14.61 7.8 13.98 7.8 13.59 7.41L13.5 7.5Z"
                      fill=""
                    />
                    <path
                      d="M6 12L7 11L8 12L7 13L6 12Z"
                      fill=""
                    />
                    <path
                      d="M10 12L11 11L12 12L11 13L10 12Z"
                      fill=""
                    />
                  </svg>
                  <span className={`transition-opacity duration-300 ${
                    sidebarExpanded ? 'opacity-100' : 'opacity-0 w-0 overflow-hidden'
                  }`}>
                    Compresseur K 244
                  </span>
                </NavLink>
              </li>
              {/* <!-- Menu Item Compresseur k244 --> */}

              {/* <!-- Menu Item ATM/MEROX & PRE-FLASH --> */}
              <li>
                <NavLink
                  to="/atm-merox-preflash"
                  className={`group relative flex items-center rounded-sm py-1.5 text-xs font-semibold text-bodydark1 duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4 ${
                    sidebarExpanded ? 'gap-2 px-3' : 'justify-center px-2'
                  } ${
                    pathname.includes('atm-merox-preflash') &&
                    'bg-graydark dark:bg-meta-4'
                  }`}
                >
                  <svg
                    className="fill-current"
                    width="14"
                    height="14"
                    viewBox="0 0 18 18"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M9 0.75C4.725 0.75 1.25 4.225 1.25 8.5C1.25 12.775 4.725 16.25 9 16.25C13.275 16.25 16.75 12.775 16.75 8.5C16.75 4.225 13.275 0.75 9 0.75ZM9 15C5.45 15 2.5 12.05 2.5 8.5C2.5 4.95 5.45 2 9 2C12.55 2 15.5 4.95 15.5 8.5C15.5 12.05 12.55 15 9 15Z"
                      fill=""
                    />
                    <path
                      d="M9 3.75C8.17 3.75 7.5 4.42 7.5 5.25V7.5C7.5 8.33 8.17 9 9 9C9.83 9 10.5 8.33 10.5 7.5V5.25C10.5 4.42 9.83 3.75 9 3.75Z"
                      fill=""
                    />
                  </svg>
                  <span className={`transition-opacity duration-300 ${
                    sidebarExpanded ? 'opacity-100' : 'opacity-0 w-0 overflow-hidden'
                  }`}>
                    ATM/MEROX & PRE-FLASH
                  </span>
                </NavLink>
              </li>
              {/* <!-- Menu Item ATM/MEROX & PRE-FLASH --> */}

              {/* <!-- Menu Item Calendar --> */}
              <li>
                <NavLink
                  to="/calendar"
                  className={`group relative flex items-center rounded-sm py-1.5 text-xs font-semibold text-bodydark1 duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4 ${
                    sidebarExpanded ? 'gap-2 px-3' : 'justify-center px-2'
                  } ${
                    pathname.includes('calendar') &&
                    'bg-graydark dark:bg-meta-4'
                  }`}
                >
                  <svg
                    className="fill-current"
                    width="14"
                    height="14"
                    viewBox="0 0 18 18"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M15.7499 2.9812H14.2874V2.36245C14.2874 2.02495 14.0062 1.71558 13.6405 1.71558C13.2749 1.71558 12.9937 1.99683 12.9937 2.36245V2.9812H4.97803V2.36245C4.97803 2.02495 4.69678 1.71558 4.33115 1.71558C3.96553 1.71558 3.68428 1.99683 3.68428 2.36245V2.9812H2.2499C1.29365 2.9812 0.478027 3.7687 0.478027 4.75308V14.5406C0.478027 15.4968 1.26553 16.3125 2.2499 16.3125H15.7499C16.7062 16.3125 17.5218 15.525 17.5218 14.5406V4.72495C17.5218 3.7687 16.7062 2.9812 15.7499 2.9812ZM1.77178 8.21245H4.1624V10.9968H1.77178V8.21245ZM5.42803 8.21245H8.38115V10.9968H5.42803V8.21245ZM8.38115 12.2625V15.0187H5.42803V12.2625H8.38115ZM9.64678 12.2625H12.5999V15.0187H9.64678V12.2625ZM9.64678 10.9968V8.21245H12.5999V10.9968H9.64678ZM13.8374 8.21245H16.228V10.9968H13.8374V8.21245ZM2.2499 4.24683H3.7124V4.83745C3.7124 5.17495 3.99365 5.48433 4.35928 5.48433C4.7249 5.48433 5.00615 5.20308 5.00615 4.83745V4.24683H13.0499V4.83745C13.0499 5.17495 13.3312 5.48433 13.6968 5.48433C14.0624 5.48433 14.3437 5.20308 14.3437 4.83745V4.24683H15.7499C16.0312 4.24683 16.2562 4.47183 16.2562 4.75308V6.94683H1.77178V4.75308C1.77178 4.47183 1.96865 4.24683 2.2499 4.24683ZM1.77178 14.5125V12.2343H4.1624V14.9906H2.2499C1.96865 15.0187 1.77178 14.7937 1.77178 14.5125ZM15.7499 15.0187H13.8374V12.2625H16.228V14.5406C16.2562 14.7937 16.0312 15.0187 15.7499 15.0187Z"
                      fill=""
                    />
                  </svg>
                  <span className={`transition-opacity duration-300 ${
                    sidebarExpanded ? 'opacity-100' : 'opacity-0 w-0 overflow-hidden'
                  }`}>
                    Calendar
                  </span>
                </NavLink>
              </li>
              {/* <!-- Menu Item Calendar --> */}

              {/* <!-- Menu Item Settings --> */}
              <li>
                <NavLink
                  to="/settings"
                  className={`group relative flex items-center rounded-sm py-1.5 text-xs font-semibold text-bodydark1 duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4 ${
                    sidebarExpanded ? 'gap-2 px-3' : 'justify-center px-2'
                  } ${
                    pathname.includes('settings') &&
                    'bg-graydark dark:bg-meta-4'
                  }`}
                >
                  <svg
                    className="fill-current"
                    width="14"
                    height="14"
                    viewBox="0 0 18 19"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <g clipPath="url(#clip0_130_9763)">
                      <path
                        d="M17.0721 7.30835C16.7909 6.99897 16.3971 6.83022 15.9752 6.83022H15.8909C15.7502 6.83022 15.6377 6.74585 15.6096 6.63335C15.5815 6.52085 15.5252 6.43647 15.4971 6.32397C15.4409 6.21147 15.4971 6.09897 15.5815 6.0146L15.6377 5.95835C15.9471 5.6771 16.1159 5.28335 16.1159 4.86147C16.1159 4.4396 15.9752 4.04585 15.6659 3.73647L14.569 2.61147C13.9784 1.99272 12.9659 1.9646 12.3471 2.58335L12.2627 2.6396C12.1784 2.72397 12.0377 2.7521 11.8971 2.69585C11.7846 2.6396 11.6721 2.58335 11.5315 2.55522C11.3909 2.49897 11.3065 2.38647 11.3065 2.27397V2.13335C11.3065 1.26147 10.6034 0.55835 9.73148 0.55835H8.15648C7.7346 0.55835 7.34085 0.7271 7.0596 1.00835C6.75023 1.31772 6.6096 1.71147 6.6096 2.10522V2.21772C6.6096 2.33022 6.52523 2.44272 6.41273 2.49897C6.35648 2.5271 6.32835 2.5271 6.2721 2.55522C6.1596 2.61147 6.01898 2.58335 5.9346 2.49897L5.87835 2.4146C5.5971 2.10522 5.20335 1.93647 4.78148 1.93647C4.3596 1.93647 3.96585 2.0771 3.65648 2.38647L2.53148 3.48335C1.91273 4.07397 1.8846 5.08647 2.50335 5.70522L2.5596 5.7896C2.64398 5.87397 2.6721 6.0146 2.61585 6.09897C2.5596 6.21147 2.53148 6.29585 2.47523 6.40835C2.41898 6.52085 2.3346 6.5771 2.19398 6.5771H2.1096C1.68773 6.5771 1.29398 6.71772 0.984604 7.0271C0.675229 7.30835 0.506479 7.7021 0.506479 8.12397L0.478354 9.69897C0.450229 10.5708 1.15335 11.274 2.02523 11.3021H2.1096C2.25023 11.3021 2.36273 11.3865 2.39085 11.499C2.4471 11.5833 2.50335 11.6677 2.53148 11.7802C2.5596 11.8927 2.53148 12.0052 2.4471 12.0896L2.39085 12.1458C2.08148 12.4271 1.91273 12.8208 1.91273 13.2427C1.91273 13.6646 2.05335 14.0583 2.36273 14.3677L3.4596 15.4927C4.05023 16.1115 5.06273 16.1396 5.68148 15.5208L5.76585 15.4646C5.85023 15.3802 5.99085 15.3521 6.13148 15.4083C6.24398 15.4646 6.35648 15.5208 6.4971 15.549C6.63773 15.6052 6.7221 15.7177 6.7221 15.8302V15.9427C6.7221 16.8146 7.42523 17.5177 8.2971 17.5177H9.8721C10.744 17.5177 11.4471 16.8146 11.4471 15.9427V15.8302C11.4471 15.7177 11.5315 15.6052 11.644 15.549C11.7002 15.5208 11.7284 15.5208 11.7846 15.4927C11.9252 15.4365 12.0377 15.4646 12.1221 15.549L12.1784 15.6333C12.4596 15.9427 12.8534 16.1115 13.2752 16.1115C13.6971 16.1115 14.0909 15.9708 14.4002 15.6615L15.5252 14.5646C16.144 13.974 16.1721 12.9615 15.5534 12.3427L15.4971 12.2583C15.4127 12.174 15.3846 12.0333 15.4409 11.949C15.4971 11.8365 15.5252 11.7521 15.5815 11.6396C15.6377 11.5271 15.7502 11.4708 15.8627 11.4708H15.9471H15.9752C16.819 11.4708 17.5221 10.7958 17.5502 9.92397L17.5784 8.34897C17.5221 8.01147 17.3534 7.5896 17.0721 7.30835ZM16.2284 9.9521C16.2284 10.1208 16.0877 10.2615 15.919 10.2615H15.8346H15.8065C15.1596 10.2615 14.569 10.6552 14.344 11.2177C14.3159 11.3021 14.2596 11.3865 14.2315 11.4708C13.9784 12.0333 14.0909 12.7365 14.5409 13.1865L14.5971 13.2708C14.7096 13.3833 14.7096 13.5802 14.5971 13.6927L13.4721 14.7896C13.3877 14.874 13.3034 14.874 13.2471 14.874C13.1909 14.874 13.1065 14.874 13.0221 14.7896L12.9659 14.7052C12.5159 14.2271 11.8409 14.0865 11.2221 14.3677L11.1096 14.424C10.4909 14.6771 10.0971 15.2396 10.0971 15.8865V15.999C10.0971 16.1677 9.95648 16.3083 9.78773 16.3083H8.21273C8.04398 16.3083 7.90335 16.1677 7.90335 15.999V15.8865C7.90335 15.2396 7.5096 14.649 6.89085 14.424C6.80648 14.3958 6.69398 14.3396 6.6096 14.3115C6.3846 14.199 6.1596 14.1708 5.9346 14.1708C5.54085 14.1708 5.1471 14.3115 4.83773 14.6208L4.78148 14.649C4.66898 14.7615 4.4721 14.7615 4.3596 14.649L3.26273 13.524C3.17835 13.4396 3.17835 13.3552 3.17835 13.299C3.17835 13.2427 3.17835 13.1583 3.26273 13.074L3.31898 13.0177C3.7971 12.5677 3.93773 11.8646 3.6846 11.3021C3.65648 11.2177 3.62835 11.1333 3.5721 11.049C3.3471 10.4583 2.7846 10.0365 2.13773 10.0365H2.05335C1.8846 10.0365 1.74398 9.89585 1.74398 9.7271L1.7721 8.1521C1.7721 8.0396 1.82835 7.98335 1.85648 7.9271C1.8846 7.89897 1.96898 7.84272 2.08148 7.84272H2.16585C2.81273 7.87085 3.40335 7.4771 3.65648 6.88647C3.6846 6.8021 3.74085 6.71772 3.76898 6.63335C4.0221 6.07085 3.9096 5.36772 3.4596 4.91772L3.40335 4.83335C3.29085 4.72085 3.29085 4.52397 3.40335 4.41147L4.52835 3.3146C4.61273 3.23022 4.6971 3.23022 4.75335 3.23022C4.8096 3.23022 4.89398 3.23022 4.97835 3.3146L5.0346 3.39897C5.4846 3.8771 6.1596 4.01772 6.77835 3.7646L6.89085 3.70835C7.5096 3.45522 7.90335 2.89272 7.90335 2.24585V2.13335C7.90335 2.02085 7.9596 1.9646 7.98773 1.90835C8.01585 1.8521 8.10023 1.82397 8.21273 1.82397H9.78773C9.95648 1.82397 10.0971 1.9646 10.0971 2.13335V2.24585C10.0971 2.89272 10.4909 3.48335 11.1096 3.70835C11.194 3.73647 11.3065 3.79272 11.3909 3.82085C11.9815 4.1021 12.6846 3.9896 13.1627 3.5396L13.2471 3.48335C13.3596 3.37085 13.5565 3.37085 13.669 3.48335L14.7659 4.60835C14.8502 4.69272 14.8502 4.7771 14.8502 4.83335C14.8502 4.8896 14.8221 4.97397 14.7659 5.05835L14.7096 5.1146C14.2034 5.53647 14.0627 6.2396 14.2877 6.8021C14.3159 6.88647 14.344 6.97085 14.4002 7.05522C14.6252 7.64585 15.1877 8.06772 15.8346 8.06772H15.919C16.0315 8.06772 16.0877 8.12397 16.144 8.1521C16.2002 8.18022 16.2284 8.2646 16.2284 8.3771V9.9521Z"
                        fill=""
                      />
                      <path
                        d="M9.00029 5.22705C6.89092 5.22705 5.17529 6.94268 5.17529 9.05205C5.17529 11.1614 6.89092 12.8771 9.00029 12.8771C11.1097 12.8771 12.8253 11.1614 12.8253 9.05205C12.8253 6.94268 11.1097 5.22705 9.00029 5.22705ZM9.00029 11.6114C7.59404 11.6114 6.44092 10.4583 6.44092 9.05205C6.44092 7.6458 7.59404 6.49268 9.00029 6.49268C10.4065 6.49268 11.5597 7.6458 11.5597 9.05205C11.5597 10.4583 10.4065 11.6114 9.00029 11.6114Z"
                        fill=""
                      />
                    </g>
                    <defs>
                      <clipPath id="clip0_130_9763">
                        <rect
                          width="14"
                          height="14"
                          fill="white"
                          transform="translate(0 0.052124)"
                        />
                      </clipPath>
                    </defs>
                  </svg>
                  <span className={`transition-opacity duration-300 ${
                    sidebarExpanded ? 'opacity-100' : 'opacity-0 w-0 overflow-hidden'
                  }`}>
                    Settings
                  </span>
                </NavLink>
              </li>
              {/* <!-- Menu Item Settings --> */}
            </ul>
          </div>

          {/* <!-- Others Group --> */}
          <div>
            <h3 className={`mb-2 text-xs font-semibold text-bodydark2 transition-opacity duration-300 ${
              sidebarExpanded ? 'ml-4 opacity-100' : 'opacity-0 h-0 mb-0 overflow-hidden'
            }`}>
              OTHERS
            </h3>

            <ul className="mb-4 flex flex-col gap-1">
              {/* <!-- Menu Item Auth Pages --> */}
              <SidebarLinkGroup
                activeCondition={
                  pathname === '/auth' || pathname.includes('auth')
                }
              >
                {(handleClick, open) => {
                  return (
                    <React.Fragment>
                      <NavLink
                        to="#"
                        className={`group relative flex items-center rounded-sm py-1.5 text-xs font-semibold text-bodydark1 duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4 ${
                          sidebarExpanded ? 'gap-2 px-3' : 'justify-center px-2'
                        } ${
                          (pathname === '/auth' || pathname.includes('auth')) &&
                          'bg-graydark dark:bg-meta-4'
                        }`}
                        onClick={(e) => {
                          e.preventDefault();
                          handleClick();
                        }}
                      >
                        <svg
                          className="fill-current"
                          width="14"
                          height="14"
                          viewBox="0 0 18 19"
                          fill="none"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <g clipPath="url(#clip0_130_9814)">
                            <path
                              d="M12.7127 0.55835H9.53457C8.80332 0.55835 8.18457 1.1771 8.18457 1.90835V3.84897C8.18457 4.18647 8.46582 4.46772 8.80332 4.46772C9.14082 4.46772 9.45019 4.18647 9.45019 3.84897V1.88022C9.45019 1.82397 9.47832 1.79585 9.53457 1.79585H12.7127C13.3877 1.79585 13.9221 2.33022 13.9221 3.00522V15.0709C13.9221 15.7459 13.3877 16.2802 12.7127 16.2802H9.53457C9.47832 16.2802 9.45019 16.2521 9.45019 16.1959V14.2552C9.45019 13.9177 9.16894 13.6365 8.80332 13.6365C8.43769 13.6365 8.18457 13.9177 8.18457 14.2552V16.1959C8.18457 16.9271 8.80332 17.5459 9.53457 17.5459H12.7127C14.0908 17.5459 15.1877 16.4209 15.1877 15.0709V3.03335C15.1877 1.65522 14.0627 0.55835 12.7127 0.55835Z"
                              fill=""
                            />
                            <path
                              d="M10.4346 8.60205L7.62207 5.7333C7.36895 5.48018 6.97519 5.48018 6.72207 5.7333C6.46895 5.98643 6.46895 6.38018 6.72207 6.6333L8.46582 8.40518H3.45957C3.12207 8.40518 2.84082 8.68643 2.84082 9.02393C2.84082 9.36143 3.12207 9.64268 3.45957 9.64268H8.49395L6.72207 11.4427C6.46895 11.6958 6.46895 12.0896 6.72207 12.3427C6.83457 12.4552 7.00332 12.5114 7.17207 12.5114C7.34082 12.5114 7.50957 12.4552 7.62207 12.3145L10.4346 9.4458C10.6877 9.24893 10.6877 8.85518 10.4346 8.60205Z"
                              fill=""
                            />
                          </g>
                          <defs>
                            <clipPath id="clip0_130_9814">
                              <rect
                                width="14"
                                height="14"
                                fill="white"
                                transform="translate(0 0.052124)"
                              />
                            </clipPath>
                          </defs>
                        </svg>
                        <span className={`transition-opacity duration-300 ${
                          sidebarExpanded ? 'opacity-100' : 'opacity-0 w-0 overflow-hidden'
                        }`}>
                          Authentication
                        </span>
                        <svg
                          className={`fill-current transition-opacity duration-300 ${
                            sidebarExpanded ? 'opacity-100 absolute right-4 top-1/2 -translate-y-1/2' : 'opacity-0 w-0 overflow-hidden'
                          } ${open && 'rotate-180'}`}
                          width="14"
                          height="14"
                          viewBox="0 0 20 20"
                          fill="none"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <path
                            fillRule="evenodd"
                            clipRule="evenodd"
                            d="M4.41107 6.9107C4.73651 6.58527 5.26414 6.58527 5.58958 6.9107L10.0003 11.3214L14.4111 6.91071C14.7365 6.58527 15.2641 6.58527 15.5896 6.91071C15.915 7.23614 15.915 7.76378 15.5896 8.08922L10.5896 13.0892C10.2641 13.4147 9.73651 13.4147 9.41107 13.0892L4.41107 8.08922C4.08563 7.76378 4.08563 7.23614 4.41107 6.9107Z"
                            fill=""
                          />
                        </svg>
                      </NavLink>
                      {/* <!-- Dropdown Menu Start --> */}
                      <div
                        className={`translate transform overflow-hidden ${
                          (!open || !sidebarExpanded) && 'hidden'
                        }`}
                      >
                        <ul className={`mt-2 mb-3 flex flex-col gap-1.5 ${sidebarExpanded ? 'pl-5' : 'pl-2'}`}>
                          <li>
                            <NavLink
                              to="/auth/signin"
                              className={({ isActive }) =>
                                `group relative flex items-center rounded-md text-xs font-semibold text-bodydark2 duration-300 ease-in-out hover:text-white ${
                                  sidebarExpanded ? 'gap-2 px-3' : 'justify-center px-2'
                                } ` +
                                (isActive && '!text-white')
                              }
                            >
                              <span className={`transition-opacity duration-300 ${
                                sidebarExpanded ? 'opacity-100' : 'opacity-0 w-0 overflow-hidden'
                              }`}>
                                Sign In
                              </span>
                            </NavLink>
                          </li>
                          <li>
                            <NavLink
                              to="/auth/signup"
                              className={({ isActive }) =>
                                `group relative flex items-center rounded-md text-xs font-semibold text-bodydark2 duration-300 ease-in-out hover:text-white ${
                                  sidebarExpanded ? 'gap-2 px-3' : 'justify-center px-2'
                                } ` +
                                (isActive && '!text-white')
                              }
                            >
                              <span className={`transition-opacity duration-300 ${
                                sidebarExpanded ? 'opacity-100' : 'opacity-0 w-0 overflow-hidden'
                              }`}>
                                Sign Up
                              </span>
                            </NavLink>
                          </li>
                        </ul>
                      </div>
                      {/* <!-- Dropdown Menu End --> */}
                    </React.Fragment>
                  );
                }}
              </SidebarLinkGroup>
              {/* <!-- Menu Item Auth Pages --> */}
            </ul>
          </div>
        </nav>
        {/* <!-- Sidebar Menu --> */}
      </div>
    </aside>
  );
};

export default Sidebar;
